...
import { editionBrowseParams, editionFilters, editionSortOptions } from "@/lib/browse-params";
...
export default function BooksPage() {
  ...
  const [params, setParams] = useQueryStates(editionBrowseParams, { shallow: false });

  const { data, isLoading } = useQuery(
    trpc.books.allBooks.queryOptions({
      page: params.page,
      search: params.q,
      sort: params.sort,
      genre: params.genre,
  ...
  return (
    <div>
      <BrowseSectionHeader title={tEntities("books")} description={t("books.description")} />

      <BrowseToolbar
        search={params.q}
        onSearchChange={(q) => setParams({ q, page: 1 })}
        searchPlaceholder={t("search.placeholder-books")}
        sort={params.sort}
